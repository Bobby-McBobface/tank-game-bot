...
export class UserCommand extends Command {
	private canvas = createCanvas(320, 200);
	...
	private drawCell(x: number, y: number, color: string) {
		...
		// Draw a rectangle with 20 by 20 pixels at the given x and y coordinates
	...
	private async drawAvatar(x: number, y: number, player: PlayersRecord) {
		...
		// Draw the image with 20 by 20 pixels at the given x and y coordinates
		this.ctx.drawImage(image, x * 20, y * 20, 20, 20);
	}
	...
	private async createGrid(players: PlayersRecord[]) {
		...
		for (let x = 0; x < 20; x++) {
			for (let y = 0; y < 20; y++) {
				...
				let color: string;
				// If the sum of x and y is even, set the color to dark green
				if ((x + y) % 2 === 0) {
					color = '#006400';
				} else {
					// Otherwise, set the color to very dark green
					color = '#003300';
				}
